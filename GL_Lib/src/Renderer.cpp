...
// Update the drawMesh method implementation
void Renderer::drawMesh(const std::vector<unsigned int>& indices, const std::vector<Texture>& textures,
                        unsigned int VAO, const glm::vec3& color)
{
    ...
    // Set up MVP matrices like other draw methods
    setUpMVP();
    ...
    // Bind textures properly for your lighting shader
    for (unsigned int i = 0; i < textures.size(); i++)
    {
        glActiveTexture(GL_TEXTURE0 + i);
        glBindTexture(GL_TEXTURE_2D, textures[i].id);

        // Set proper uniform names based on your fragment shader
        if (textures[i].type == "diffuse")
        {
            int diffuseLoc = glGetUniformLocation(prog, "material.diffuse");
            if (diffuseLoc != -1)
                glUniform1i(diffuseLoc, i);
        }
        else if (textures[i].type == "specular")
        {
            int specularLoc = glGetUniformLocation(prog, "material.specular");
            if (specularLoc != -1)
                glUniform1i(specularLoc, i);
        }
    }
