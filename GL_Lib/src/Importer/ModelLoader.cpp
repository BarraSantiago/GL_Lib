...
namespace gllib
{
    ...
    void ModelLoader::loadModel(std::string const& path, std::vector<Mesh>& meshes, bool gamma)
    {
        ...
        processNode(scene->mRootNode, scene, meshes, gamma, minAABB, maxAABB);
    }

    void ModelLoader::processNode(aiNode* node, const aiScene* scene, std::vector<Mesh>& meshes, bool gamma, glm::vec3& minAABB, glm::vec3& maxAABB)
    {
